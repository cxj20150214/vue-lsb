  ...
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" width="290" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary"  @click="handleUpdate(scope.row)">编辑</el-button>
           ...
           <el-button type="success"  @click="handleAllSta(scope.row)">统计</el-button>
    ...
    >
      ...
      >
        <el-input
          ...
          style="width:0px;height:0px;"
        />
          <el-form-item label="盖章品牌" prop="stamp_brand_id">
          <el-input v-model="tempStamp.stamp_brand_id " placeholder="请输入盖章品牌" type="hidden"
          style="width:0px;height:0px;"/>
          <el-select v-model="tempStamp.stamp_brand_id" placeholder="请选择" @change="getcustomerjl">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="盖章经理" prop="stamp_user_id">
          <el-input v-model="tempStamp.stamp_user_id " placeholder="请输入盖章经理" type="hidden"
          style="width:0px;height:0px;"/>
          <el-select v-model="tempStamp.stamp_user_id" placeholder="请选择">
            <el-option
              v-for="item in customerjl"
              :key="item.id"
              :label="item.realname"
              :value="item.id"
            ></el-option>
        ...
        <!-- <el-form-item label="盖章品牌id" prop="stamp_brand_id">
          <el-input v-model="tempStamp.stamp_brand_id " placeholder="请输入盖章品牌id" type="hidden"
          style="width:0px;height:0px;"/>
          <el-select v-model="tempStamp.stamp_brand_id" placeholder="请选择" >
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>-->
         <el-form-item label="客户" prop="customer_id">
          <el-input v-model="tempStamp.customer_id " placeholder="请输入客户id " type="hidden"
          style="width:0px;height:0px;"/>
          <el-select v-model="tempStamp.customer_id" placeholder="请选择">
            <el-option
         ...
         <el-form-item label="客户意向" prop="intention">
          <el-select v-model="tempStamp.intention" placeholder="请选择">
            <el-option
              v-for="item in options_intent"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus==='create'?createDataStamp():updateDataStamp()">确定</el-button>
      </div>
    </el-dialog>
    <!--新增回访-->
    <el-dialog
      :title="textMap[dialogStatus]"
      :visible.sync="dialogFormVisibleReviewadd"
      :close-on-press-escape="dialogEsc"
      :close-on-click-modal="dialogEsc"
      width
    >
      <el-form
        ref="dataFormReviewadd"
        :rules="rulesReview"
        :model="tempReview"
        label-position="left"
        label-width="80px"
      ...
      >
        <el-input
          v-model="tempReview.id"
          placeholder="请输入活动id"
          type="hidden"
          style="width:0px;height:0px;"
        />
        <el-form-item label="客户" prop="customer_id">
          <el-input v-model="tempReview.customer_id" placeholder="请输入客户" type="hidden"
          style="width:0px;height:0px;"/>
          <el-select v-model="tempReview.customer_id" placeholder="请选择">
            <el-option
              v-for="item in customerId"
              :key="item.id"
              :label="item.name"
        ...
        <el-form-item label="产品需求" prop="demands">
          <el-checkbox-group v-model="tempReview.demands" @change="checkList1">
            <el-checkbox
         ...
         <el-form-item label="客户来源" prop="source">
          <el-select v-model="tempReview.source" placeholder="请选择">
            <el-option
        ...
        <el-form-item label="客户进度" prop="progress">
          <el-input v-model="tempReview.progress" placeholder="请输入客户进度" />
        </el-form-item>
        <el-form-item label="品牌" prop="brand_id">
        <el-input v-model="tempReview.brand_id" placeholder="请输入品牌" type="hidden" style="width:0px;height:0px;"/>
        <el-select v-model="tempReview.brand_id" placeholder="请选择" @change="getcustomerjl">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="客户经理" prop="user_id">
          <el-input v-model="tempReview.user_id" placeholder="请输入客户经理" type="hidden" style="width:0px;height:0px;"/>
          <el-select v-model="tempReview.user_id" placeholder="请选择">
            <el-option
              v-for="item in customerjl"
              :key="item.id"
              :label="item.realname"
        ...
        <el-form-item label="到场情况" prop="present_situation">
          <el-input v-model="tempReview.present_situation" placeholder="请输入到场情况" />
        </el-form-item>
        <el-form-item label="到场人数" prop="turnout">
          <el-input v-model="tempReview.turnout" placeholder="请输入到场人数" />
        </el-form-item>
        <el-form-item label="其他说明" prop="explain">
          <el-input v-model="tempReview.explain" placeholder="请输入其他说明" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisibleReviewadd = false">取消</el-button>
        <el-button type="primary" @click="dialogStatus==='create'?createDataReview():updateDataReview()">确定</el-button>
      </div>
    </el-dialog>
    <!--新增客户-->
    <el-dialog
      title="新增"
      :visible.sync="dialogFormVisibleCustomeradd"
      :close-on-press-escape="dialogEsc"
      :close-on-click-modal="dialogEsc"
      width
    >
      <el-form
        ref="dataFormCustomeradd"
        :rules="rulesCustomer"
        :model="tempCustomer"
        label-position="left"
        label-width="80px"
      ...
      >
        ...
        />
        <el-form-item label="品牌" prop="brand_id">
        <el-input v-model="tempCustomer.brand_id" placeholder="请输入品牌" type="hidden" style="width:0px;height:0px;"/>
        <el-select v-model="tempCustomer.brand_id" placeholder="请选择" @change="getcustomerjl">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="条形码" prop="qrcode">
          <el-input v-model="tempCustomer.qrcode" placeholder="请输入条形码" />
        </el-form-item>
        ...
        <el-form-item label="客户来源" prop="source_key">
        ...
        <el-form-item label="客户经理" prop="user_id">
          <el-select v-model="tempCustomer.user_id" placeholder="请选择">
            <el-option
              v-for="item in customerjl"
              :key="item.id"
              :label="item.realname"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisibleCustomeradd = false">取消</el-button>
        <el-button type="primary" @click="createDataCustomer()">确定</el-button>
      </div>
    </el-dialog>
    <el-dialog
      :title="textMap[dialogStatus]"
    ...
    >
      ...
      >
        ...
        />
         <el-form-item label="客户" prop="customer_id">
          <el-input v-model="tempOrder.customer_id" placeholder="请输入客户" type="hidden"
          style="width:0px;height:0px;"/>
          <el-select v-model="tempOrder.customer_id" placeholder="请选择">
            <el-option
              v-for="item in customerId"
              :key="item.id"
              :label="item.name"
         ...
         <el-form-item label="定单品牌" prop="order_brand_id">
          <el-input v-model="tempOrder.order_brand_id  " placeholder="请输入定单品牌id"    type="hidden"
          style="width:0px; height:0px;"/>
          <el-select v-model="tempOrder.order_brand_id" placeholder="请选择" @change="getcustomerjl">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
         <el-form-item label="定单经理" prop="order_user_id">
          <el-input v-model="tempOrder.order_user_id  " placeholder="请输入定单经理id"    type="hidden"
          style="width:0px; height:0px;"/>
          <el-select v-model="tempOrder.order_user_id" placeholder="请选择">
            <el-option
              v-for="item in customerjl"
              :key="item.id"
              :label="item.realname"
         ...
         <el-form-item label="售卡品牌" prop="card_brand_id">
          <el-input v-model="tempOrder.card_brand_id  " placeholder="请输入售卡品牌id"    type="hidden"
          style="width:0px; height:0px;"/>
          <el-select v-model="tempOrder.card_brand_id" placeholder="请选择" @change="getcustomerjl">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
         <el-form-item label="售卡经理" prop="card_user_id">
          <el-input v-model="tempOrder.card_user_id  " placeholder="请输入售卡经理id"    type="hidden"
          style="width:0px; height:0px;"/>
          <el-select v-model="tempOrder.card_user_id" placeholder="请选择">
            <el-option
              v-for="item in customerjl"
              :key="item.id"
              :label="item.realname"
        ...
        <el-form-item label="带单品牌" prop="from_brand_id">
          <el-select v-model="tempOrder.from_brand_id" placeholder="请选择" @change="getcustomerjl">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-input
    ...
    <el-dialog
      title="数据报表"
      :visible.sync="dialogAllSta"
      width="95%"
      >
    <template>
      <el-tabs v-model="activeNameSta" @tab-click="handleClickSta">
        <el-tab-pane label="售卡进度表" name="first1">
         <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime1"
      ...
      <el-table-column  v-for="item in dateListsta1" :label="item"  align="center" width="100">
      </el-table-column>
      <el-table-column
      label="合计"
      width="100"
      ...
      <template slot-scope="scope">
      <ul class="tr_sta1">
          <li v-for="item in scope.row.data">{{item}}</li>
      </ul>
      </template>
      </el-table-column>
      </el-table>
      <!-- <div class="table_tr">
        <tr>
          <th>品牌</th>
          <th style="width:100px;" v-for="item in dateListsta1">{{item}}</th>
        </tr>
      </div> -->
        </el-tab-pane>
        <el-tab-pane label="定单统计表" name="first2">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime2"
          ...
          <el-table-column  v-for="item in dateListsta2" :label="item"  align="center" width="100">
             <el-table-column
               label="售卡"
               width="100"
               align="center"
        ...
        <el-tab-pane label="日数据汇总表" name="first3">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime3"
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_card}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="总计售卡"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_card}}</span>
               </template>
              </el-table-column>
          </el-table-column>
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="今日带单数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_from_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="今日被带单数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_by_from_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="总计定单数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="总计带单数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_from_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="总计被带单数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_by_from_order}}</span>
               </template>
              </el-table-column>
          </el-table-column>
        ...
        <el-table-column
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_card_to_stamp}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="合计售卡客户去其他店盖章数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_card_to_stamp}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="累计客户进店盖章数"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_stamp}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="进店客户成交率"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.closing_ratio}}</span>
               </template>
              </el-table-column>
         ...
         <el-tab-pane label="（新）日数据汇总表" name="first4">
         <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime4"
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_own_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="今日被带单数"
        ...
        <el-table-column
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_own_order}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="总计被带单数"
         ...
         <el-tab-pane label="（新）盖章统计表" name="first5">
         <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime5"
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_stamp_customer}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="盖章"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_stamp}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="盖章3个以上"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.today_stamp_3}}</span>
               </template>
              </el-table-column>
          </el-table-column>
        ...
        <el-table-column
          ...
          >
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_stamp_customer}}</span>
               </template>
              </el-table-column>
              <el-table-column
              label="累计盖章"
              ...
              >
              <template slot-scope="scope">
                  <span>{{ scope.row.data.all_stamp_3}}</span>
               </template>
              </el-table-column>
          </el-table-column>
           </el-table>
        ...
        <el-tab-pane label="贡献单统计表" name="first6">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime6"
        ...
        <el-tab-pane label="联购统计表" name="first7">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime7"
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.union_buy_brand_num}}</span>
                  <ul class="tr_sta3">
                    <li v-for="item in scope.row.brand_data">{{item.order_num}}</li>
                  </ul>
            </template>
          </el-table-column>
        ...
        <el-tab-pane label="活动数据总表" name="first8">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime8"
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.order_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.sign_customer_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_order_customer_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_order_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.sign_ratio}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.sign_order_ratio}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_order_ratio}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.customer_average_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_own_order_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_customer_other_order_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.other_customer_order_num}}</span>
            </template>
          </el-table-column>
         ...
         <el-tab-pane label="（新）联购统计表" name="first9">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime9"
        ...
        <el-tab-pane label="（新）带单统计" name="first10">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime10"
            ...
            <el-select class="select_ss filter-item" v-model="sta10listQuery.brand_id" clearable placeholder="请选择品牌" @change="staFilter10">
              <el-option v-for="item in options5" :key="item.brand_id" :label="item.brand_name" :value="item.brand_id"></el-option>
            </el-select>
            <el-button
              v-waves
              class="filter-item"
         ...
         <el-table-column
          ...
          >
            <template slot-scope="scope">
                  <ul class="tr_sta2">
                    <li v-for="item in scope.row.data">{{item.num}}</li>
                  </ul>
            </template>
          </el-table-column>
           </el-table>
        ...
        <el-tab-pane label="客户盖章统计表" name="first11">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime11"
          ...
          >
            <template slot-scope="scope">
                  <ul class="tr_sta4">
                    <li v-for="item in scope.row.data">{{item.intention}}</li>
                  </ul>
            </template>
          </el-table-column>
         <el-table-column
          ...
          width="150"
          align="center"
          v-for="item in lieliststa11"
          >
          </el-table-column>
           </el-table>
        </el-tab-pane>

         <el-tab-pane label="员工数据统计表" name="first12">
         <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime12"
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_customer_stamp_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_customer_stamp_ratio}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_customer_sign_num}}</span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.card_customer_sign_ratio}}</span>
            </template>
          </el-table-column>
           <el-table-column
          label="售卡客户定单户数"
          ...
          >
            <template slot-scope="scope">
                  <span>{{ scope.row.data.from_order_num}}</span>
            </template>
          </el-table-column>
           </el-table>
        </el-tab-pane>

        <el-tab-pane label="客户签到统计表" name="first13">
        <div class="filter-container">
           <el-date-picker
              v-model="value_time"
              @change="staTime13"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="dialogAllSta = false">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 总列表 -->
    <el-dialog
  ...
  <el-tabs v-model="activeName" @tab-click="handleClick">
    ...
    <div class="filter-container">
        <el-input
          ...
          clearable
          style="width: 200px;"
          class="filter-item ss"
          placeholder="真实姓名"
          @keyup.enter.native="handleFilter2"
        />
        <el-select class="select_ss filter-item" v-model="AdminlistQuery.brand_id" clearable placeholder="请选择品牌" @change="handleFilter2">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-select
          v-model="AdminlistQuery.store_id"
          filterable
          remote
          reserve-keyword
          class="filter-item select_ss"
          placeholder="请输入门店"
        ...
        <el-input
          v-model="AdminlistQuery.date_range"
          clearable
          style="width: 0px;"
          class="filter-item ss"
          placeholder="日期范围"
    ...
    <el-tab-pane label="定单列表" name="first2">
     <div class="filter-container">
        ...
        <el-input
          ...
          @keyup.enter.native="handleFilter1"
        />
        <el-select class="select_ss filter-item" v-model="OrderlistQuery.order_brand_id" clearable placeholder="请选择定单品牌" @change="handleFilter1">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-select
          v-model="OrderlistQuery.store_id"
          filterable
          remote
          reserve-keyword
          class="filter-item select_ss"
         ...
         <el-input
          v-model="CustomerlistQuery.date_range"
          clearable
          style="width: 0px;"
          class="filter-item ss"
          placeholder="日期范围"
    ...
    <el-tab-pane label="客户列表" name="first3">
     <div class="filter-container">
      <el-input
          v-model="CustomerlistQuery.name"
          clearable
          style="width: 200px;"
          class="filter-item ss"
          placeholder="客户姓名"
          @keyup.enter.native="handleFilter5"
        />
        <el-input
          v-model="CustomerlistQuery.qrcode"
          clearable
          style="width: 200px;"
          class="filter-item ss"
          placeholder="条形码"
        ...
        />
        <el-input
          v-model="CustomerlistQuery.mobile"
          clearable
          style="width: 200px;"
          class="filter-item ss"
          placeholder="电话"
        ...
        />
        <el-input
          v-model="CustomerlistQuery.addr"
          clearable
          style="width: 200px;"
          class="filter-item ss"
          placeholder="地址"
          @keyup.enter.native="handleFilter5"
        />
        <el-select class="select_ss filter-item" v-model="CustomerlistQuery.brand_id" clearable placeholder="请选择品牌" @change="handleFilter5">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-select
          v-model="CustomerlistQuery.store_id"
          filterable
          remote
          reserve-keyword
          class="filter-item select_ss"
    ...
    <div class="filter-container">
        ...
        />
        <el-select class="select_ss filter-item" v-model="OrderlistSignQuery.order_brand_id" clearable placeholder="请选择品牌" @change="handleFilter3">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-select
          v-model="OrderlistSignQuery.store_id"
          filterable
          remote
          reserve-keyword
          class="filter-item select_ss"
         ...
         <el-input
          v-model="OrderlistSignQuery.date_range"
          clearable
          style="width: 0px;"
          class="filter-item ss"
          placeholder="日期范围"
    ...
    <el-tab-pane label="回访列表" name="first5">
     <div class="filter-container">
        ...
        />
        <el-select class="select_ss filter-item" v-model="OrderlistReviewQuery.order_brand_id" clearable placeholder="请选择品牌" @change="handleFilter4">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-select
          v-model="OrderlistReviewQuery.store_id"
          filterable
          remote
          reserve-keyword
          class="filter-item select_ss"
         ...
         <el-input
          v-model="OrderlistReviewQuery.date_range"
          clearable
          style="width: 0px;"
          class="filter-item ss"
          placeholder="日期范围"
    ...
    <el-date-picker
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="handleUpdateReview(scope.row)">编辑</el-button>
     ...
     <div class="filter-container">
      ...
      >
        ...
        <el-table-column label="排序" prop="sort" align="center" width="80">
          <template slot-scope="scope">
           <el-input @blur="saveSort(scope.row)" v-model="scope.row.sort" align="center"/>
          </template>
        </el-table-column>
      ...
      <el-button @click="handleSort()" type="primary" style="margin-top:20px" >批量更新排序</el-button>
      <pagination
        v-show="total>0"
        :total="total"
        :page.sync="brandlistQuery.page"
        :limit.sync="brandlistQuery.limit"
    ...
    <pagination
        v-show="total>0"
        :total="total"
        :page.sync="DemandlistQuery.page"
        :limit.sync="DemandlistQuery.limit"
        @pagination="demandList"
      />
    </el-tab-pane>
    <el-tab-pane label="盖章信息" name="first8">
    <div class="filter-container">
      <el-input
          v-model="StamplistQuery.name"
          clearable
          style="width: 200px;"
          class="filter-item ss"
          placeholder="客户姓名"
          @keyup.enter.native="handleFilter6"
        />
        <el-select class="select_ss filter-item" v-model="StamplistQuery.stamp_brand_id" clearable placeholder="请选择品牌" @change="handleFilter6">
            <el-option v-for="item in options2" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-select
          v-model="StamplistQuery.store_id"
          filterable
          remote
          reserve-keyword
          class="filter-item select_ss"
         ...
         <el-input
          v-model="StamplistQuery.date_range"
          clearable
          style="width: 0px;"
          class="filter-item ss"
          placeholder="日期范围"
    ...
    <el-button
      ...
      >
        ...
        v-show="total>0"
        :total="total"
        :page.sync="StamplistQuery.page"
        :limit.sync="StamplistQuery.limit"
        @pagination="StampList"
      />
    </el-tab-pane>
  </el-tabs>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      MDoptions: [],
        ...
        MDlist: [],
        MDloading: false,
        MDstates: ["Alabama", "Alaska", "Arizona",
      ...
      dialogFormVisibleReviewadd:false,
       dialogAll: false,
      // 管理员状态
      options_shop:[
        {
      ...
      dialogFormVisibleCustomeradd:false,
      ...
      rulesOrder: {
        ...
        addr: [
          { required: true, message: "地址必填项", trigger: "blur" }
        ],
      ...
      rulesReview: {
        customer_id: [{ required: true, message: "客户必填项", trigger: "blur" }],
        demands : [{ required: true, message: "需求必填项", trigger: "blur" }],
        source  : [{ required: true, message: "来源必填项", trigger: "blur" }],
        progress : [{ required: true, message: "进度必填项", trigger: "blur" }],
        brand_id : [{ required: true, message: "品牌必填项", trigger: "blur" }],
        user_id : [{ required: true, message: "经理必填项", trigger: "blur" }],
        present_situation : [{ required: true, message: "到场情况必填项", trigger: "blur" }],
        turnout : [{ required: true, message: "到场人数必填项", trigger: "blur" }],
        explain : [{ required: true, message: "其他说明必填项", trigger: "blur" }],
      ...
      rulesStamp: {
        ...
        stamp_user_id : [{ required: true, message: "售卡经理必填项", trigger: "blur" }],
        brand_id : [{ required: true, message: "售卡品牌必填项", trigger: "blur" }],
        stamp_brand_id : [{ required: true, message: "盖章品牌必填项", trigger: "blur" }],
        customer_id : [{ required: true, message: "客户必填项", trigger: "blur" }],
      }
    };
  },
  mounted(){
    // 搜索门店
      this.MDlist = this.MDstates.map(item => {
  ...
  methods: {
    ...
    findStore(value){
      console.log(value)
      var thisStoreID = value
      search_store_brand({
        store_id: thisStoreID
      }).then(res=>{
        console.log(res.data)
        this.tempBrand.name = res.data.brand_name;
      })
    },
    // 搜索时间
     selectTime(val){
      if (val != null) {
       this.AdminlistQuery.date_range = val[0] + ' - ' + val[1];
      }else{
        this.AdminlistQuery.date_range =''
      }
      console.log(this.AdminlistQuery.date_range);
      this.handleFilter2();
    },
    selectTime1(val){
       if (val != null) {
       this.CustomerlistQuery.date_range = val[0] + ' - ' + val[1];
      }else{
        this.CustomerlistQuery.date_range =''
      }
      console.log(this.CustomerlistQuery.date_range);
      this.handleFilter5();
    },
    selectTime2(val){
       if (val != null) {
       this.OrderlistQuery.date_range = val[0] + ' - ' + val[1];
      }else{
        this.OrderlistQuery.date_range =''
      }
      console.log(this.OrderlistQuery.date_range);
      this.handleFilter1();
    },
     selectTime3(val){
       if (val != null) {
     ...
     selectTime4(val){
       if (val != null) {
       this.OrderlistReviewQuery.date_range = val[0] + ' - ' + val[1];
      }else{
        this.OrderlistReviewQuery.date_range =''
      }
      console.log(this.OrderlistReviewQuery.date_range);
      this.handleFilter4();
    },
     selectTime5(val){
       if (val != null) {
       this.StamplistQuery.date_range = val[0] + ' - ' + val[1];
      }else{
        this.StamplistQuery.date_range =''
      }
      console.log(this.StamplistQuery.date_range);
      this.handleFilter6();
    },
    // 统计搜索日期
    staTime1(val){
      if (val != null) {
       this.sta1listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta1listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter1();
    },
     staTime2(val){
      if (val != null) {
       this.sta2listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta2listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter2();
     ...
     staTime3(val){
      if (val != null) {
       this.sta3listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta3listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter3();
     ...
     staTime4(val){
      if (val != null) {
       this.sta4listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta4listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter4();
     ...
     staTime5(val){
      if (val != null) {
       this.sta5listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta5listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter5();
     ...
     staTime6(val){
      if (val != null) {
       this.sta6listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta6listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter6();
     ...
     staTime7(val){
      if (val != null) {
       this.sta7listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta7listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter7();
     ...
     staTime8(val){
      if (val != null) {
       this.sta8listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta8listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter8();
     ...
     staTime9(val){
      if (val != null) {
       this.sta9listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta9listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter9();
     ...
     staTime10(val){
      if (val != null) {
       this.sta10listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta10listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter10();
     ...
     staTime11(val){
      if (val != null) {
       this.sta11listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta11listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter11();
     ...
     staTime12(val){
      if (val != null) {
       this.sta12listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta12listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter12();
     ...
     staTime13(val){
      if (val != null) {
       this.sta13listQuery.date_range = val[0] + ' ' + val[1];
      }else{
        this.sta13listQuery.date_range =''
      }
      console.log(val,'测试日期');
      // console.log(this.AdminlistQuery.date_range);
      this.staFilter13();
     ...
     remoteMethod(query) {
        if (query !== '') {
          ...
          getmdlist({
            ...
            console.log(res,'搜索门店列表')
            this.MDoptions = res.data.list
          ...
          // setTimeout(() => {
          //   this.MDloading = false;
          //   this.MDoptions = this.MDlist.filter(item => {
    ...
    getcustomerId(){
       getCustomerId({
        activity_id:  this.OrderlistQuery.activity_id,
      }).then(response => {
        this.customerId = response.data.list
        console.log(response.data.list,'获取定单客户列表');
      });
    },
     // 更新活动品牌排序
    saveSort(row){
      var sortdataobj={};
      var brandId = row.id
      var brandSort = parseInt(row.sort);
      sortdataobj['id'] = brandId;
      sortdataobj['sort'] = brandSort;
      this.sortdata.push(sortdataobj);
      console.log(this.sortdata)
    },
    handleSort(){
       saveSort({
        id: this.brandlistQuery.activity_id,
        sortdata:this.sortdata
      }).then(response => {
        console.log(response,"更新活动品牌排序");
        this.brandList();
      });
    },
    // 获取客户经理列表
    getcustomerjl(value){
      console.log(value,'客户经理')
      var thisid = value
      getCustomerjl({
        activity_id:  this.CustomerlistQuery.activity_id,
        brand_id:value
      }).then(response => {
        this.customerjl = response.data.list
        console.log(response.data.list,'获取客户经理列表');
      });
    },
    // 获取门店列表
    removeform(){
    ...
    addformCustomer(){
       this.rulesCustomer = {
         source_key : [{ required: true, message: "客户来源必填项", trigger: "blur" }],
         user_id: [{ required: true, message: "客户经理必填项", trigger: "blur" }],
         brand_id: [{ required: true, message: "品牌必填项", trigger: "blur" }],
          qrcode : [{ required: true, message: "条形码必填项", trigger: "blur" }],
        name: [{ required: true, message: "名字必填项", trigger: "blur" }],
        mobile: [{ required: true, message: "手机号必填项", trigger: "blur" }],
        sex: [{ required: true, message: "性别必填项", trigger: "blur" }],
        demand_ids: [
    ...
    // 导出表格统计
    exportExcelsta1 () {
       var activityId = this.sta1listQuery.activity_id;
    ...
    exportExcelsta2 () {
       var activityId = this.sta2listQuery.activity_id;
       console.log(activityId)
       sta2({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta3 () {
       var activityId = this.sta3listQuery.activity_id;
       console.log(activityId)
       sta3({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta4 () {
       var activityId = this.sta4listQuery.activity_id;
       console.log(activityId)
       sta4({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta5 () {
       var activityId = this.sta5listQuery.activity_id;
       console.log(activityId)
       sta5({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta6 () {
       var activityId = this.sta6listQuery.activity_id;
       console.log(activityId)
       sta6({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta7 () {
       var activityId = this.sta7listQuery.activity_id;
       console.log(activityId)
       sta7({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta8 () {
       var activityId = this.sta8listQuery.activity_id;
       console.log(activityId)
       sta8({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta9 () {
       var activityId = this.sta9listQuery.activity_id;
       console.log(activityId)
       sta9({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta10 () {
       var activityId = this.sta10listQuery.activity_id;
       console.log(activityId)
       sta10({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta11 () {
       var activityId = this.sta11listQuery.activity_id;
       console.log(activityId)
       sta11({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta12 () {
       var activityId = this.sta12listQuery.activity_id;
       console.log(activityId)
       sta12({
         activity_id : activityId,
         export :1
    ...
    exportExcelsta13 () {
       var activityId = this.sta13listQuery.activity_id;
       console.log(activityId)
       sta13({
         activity_id : activityId,
         export :1
    ...
    // 导出表格
    exportExcel () {
       var activityId = this.OrderlistSignQuery.activity_id;
       console.log(activityId)
       adminExport({
         activity_id :activityId
       }).then(res=>{
         console.log(res.data.down_url)
   ...
   var activityId = this.OrderlistSignQuery.activity_id;
    ...
    // 获取统计的各个列表
     sta1(row) {
      ...
      console.log("sta1")
      sta1(this.sta1listQuery).then(response => {
        console.log(response , '售卡进度表');
        this.liststa1 = response.data.list;
        this.dateListsta1 =response.data.date_list;
        this.value_time = response.data.date_range;
        console.log(this.value_time,'日期范围');
    ...
    sta2(row) {
      ...
      this.listLoading2 = true;
      console.log("sta2")
      sta2(this.sta2listQuery).then(response => {
        console.log(response , '定单统计');
        this.liststa2 = response.data.list;
        this.dateListsta2 =response.data.date_list;
        this.value_time = response.data.date_range;
        console.log(this.dateListsta2,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta3(row) {
      ...
      this.listLoading2 = true;
      console.log("sta3")
      sta3(this.sta3listQuery).then(response => {
        console.log(response , '日数据汇总表');
        this.liststa3 = response.data.list;
        this.value_time = response.data.date_range;
        console.log(this.liststa3,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta4(row) {
      ...
      this.listLoading2 = true;
      console.log("sta4")
      sta4(this.sta4listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa4 = response.data.list;
        this.value_time = response.data.date_range;
        console.log(this.liststa4,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta5(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta5(this.sta5listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa5 = response.data.list;
        this.value_time = response.data.date_range;
        console.log(this.liststa5,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta6(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta6(this.sta6listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa6 = response.data.list;
        this.lieliststa6 =  response.data.brand_list
        this.value_time = response.data.date_range;
        console.log(this.liststa6,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta7(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta7(this.sta7listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa7 = response.data.list;
        this.lieliststa7 =  response.data.brand_list
        this.value_time = response.data.date_range;
        console.log(this.liststa7,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta8(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta8(this.sta8listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa8 = response.data.list;
        this.lieliststa8 =  response.data.brand_data
        this.value_time = response.data.date_range;
        console.log(this.liststa8,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta9(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta9(this.sta9listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa9 = response.data.list;
        this.lieliststa9 =  response.data.brand_num_list;
        this.value_time = response.data.date_range;
        console.log(this.liststa9,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta10(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta10(this.sta10listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa10 = response.data.list;
        this.lieliststa10 =  response.data.brand_list;
        this.value_time = response.data.date_range;
        this.options5 = response.data.brand_list;
        console.log(this.liststa10,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta11(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta11(this.sta11listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa11 = response.data.list;
        this.lieliststa11 =  response.data.brand_list
        this.value_time = response.data.date_range;
        console.log(this.liststa11,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta12(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta12(this.sta12listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa12 = response.data.list;
        this.lieliststa12 =  response.data.brand_list
        this.value_time = response.data.date_range;
        console.log(this.liststa12,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    sta13(row) {
      ...
      this.listLoading2 = true;
      console.log("sta5")
      sta13(this.sta13listQuery).then(response => {
        console.log(response , '（新）日数据汇总表');
        this.liststa13 = response.data.list;
        this.lieliststa13 =  response.data.brand_list
        this.value_time = response.data.date_range;
        console.log(this.liststa13,'111');
        // Just to simulate the time of the request
        setTimeout(() => {
          this.listLoading2 = false;
        }, 0.5 * 1000);
    ...
    handleFilter5() {
      this.CustomerlistQuery.page = 1;
      this.CustomerList();
    },
    handleFilter6() {
      this.StamplistQuery.page = 1;
      this.StampList();
    },
    staFilter1() {
      this.sta1();
    ...
    // 回访新增
     handleCreateReview() {
      ...
      this.getcustomerId();
      this.dialogStatus = "create";
      this.dialogFormVisibleReviewadd = true;
      this.$nextTick(() => {
        this.$refs["dataFormReviewadd"].clearValidate();
      });
    },
    // 客户新增
     handleCreateCustomer() {
      ...
      this.resetTempCustomer();
      this.getPaytypeidCust();
      this.dialogStatus = "create";
      this.dialogFormVisibleCustomeradd = true;
      this.$nextTick(() => {
        this.$refs["dataFormCustomeradd"].clearValidate();
      });
    },
    // 管理员新增
    handleCreateAdmin() {
    ...
    createDataStamp() {
      this.$refs["dataFormStamp"].validate(valid => {
        if (valid) {
          // this.temp.routes = this.$refs.tree.getCheckedKeys();
          this.tempStamp.activity_id = this.StamplistQuery.activity_id;
          Stampadd(this.tempStamp).then(response => {
            console.log(response);
            this.Stamplist.unshift(response.data);
       ...
       // 回访新增(确定)
    createDataReview() {
      this.$refs["dataFormReviewadd"].validate(valid => {
        if (valid) {
          // this.temp.routes = this.$refs.tree.getCheckedKeys();
          this.tempReview.activity_id = this.OrderlistReviewQuery.activity_id;
          Reviewadd(this.tempReview).then(response => {
            console.log(response);
            this.OrderlistReview.unshift(response.data);
            this.dialogFormVisibleReviewadd = false;
            this.$notify({
              title: "成功",
              message: "创建成功",
              type: "success",
     ...
     // 客户新增(确定)
    createDataCustomer() {
      this.$refs["dataFormCustomeradd"].validate(valid => {
        if (valid) {
          // this.temp.routes = this.$refs.tree.getCheckedKeys();
          this.tempCustomer.activity_id = this.CustomerlistQuery.activity_id;
          Customeradd(this.tempCustomer).then(response => {
            console.log(response);
            this.Customerlist.unshift(response.data);
            this.dialogFormVisibleCustomeradd = false;
            this.$notify({
              title: "成功",
              message: "创建成功",
              type: "success",
    ...
    // 定单新增(确定)
    createDataOrder() {
      this.$refs["dataFormOrderadd"].validate(valid => {
        if (valid) {
          // this.temp.routes = this.$refs.tree.getCheckedKeys();
    ...
    handleAll(row){
     ...
     handleAllSta(row){
      this.activeNameSta = "first1"
      this.dialogAllSta = true
      this.sta1(row);
      if (row == undefined) {
        console.log("搜索");
      }
      if (row != undefined) {
        console.log(row.id,'row不为空')
        this.sta1listQuery.activity_id = row.id;
        this.sta2listQuery.activity_id = row.id;
        this.sta3listQuery.activity_id = row.id;
        this.sta4listQuery.activity_id = row.id;
        this.sta5listQuery.activity_id = row.id;
        this.sta6listQuery.activity_id = row.id;
        this.sta7listQuery.activity_id = row.id;
        this.sta8listQuery.activity_id = row.id;
        this.sta9listQuery.activity_id = row.id;
        this.sta10listQuery.activity_id = row.id;
        this.sta11listQuery.activity_id = row.id;
        this.sta12listQuery.activity_id = row.id;
        this.sta13listQuery.activity_id = row.id;
      }
      // this.orderList(row);
      // this.CustomerList(row);
      // this.ordersignList(row);
    ...
    handleUpdate1(row) {
      ...
      var value = this.tempOrder.order_brand_id
      getCustomerjl({
        activity_id:  this.OrderlistQuery.activity_id,
        brand_id:value
      }).then(response => {
    ...
    // 编辑回访信息
     handleUpdateReview(row) {
      console.log(row, "回访列表编辑");
      console.log(this.OrderlistReviewQuery.activity_id,"回访列表id")
      
      this.dialogStatus = "update";
      this.dialogFormVisibleReviewadd = true;
      console.log(this.OrderlistQuery.activity_id);
      this.tempReview = Object.assign({}, row);
      var value = this.tempReview.brand_id
      getCustomerjl({
        activity_id:  this.CustomerlistQuery.activity_id,
        brand_id:value
      }).then(response => {
     ...
     updateDataReview() {
      this.$refs["dataFormReviewadd"].validate(valid => {
        if (valid) {
          // this.temp.routes = this.$refs.tree.getCheckedKeys();
          this.tempReview.activity_id = this.OrderlistReviewQuery.activity_id;
          const tempData = Object.assign({}, this.tempReview); //复制form的数据到当前提交的接口
          console.log(this.tempReview)
          Reviewsave(tempData).then(response => {
            if (typeof response !== "undefined") {
              for (const v of this.OrderlistReview) {
                if (v.id === this.tempReview.id) {
                  const index = this.OrderlistReview.indexOf(v);
                  this.OrderlistReview.splice(index, 1, this.tempReview);
                  break;
                }
              }
              this.dialogFormVisibleReviewadd = false;
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteReview(row).then(() => {
            ...
            const index = this.OrderlistReview.indexOf(row);
            this.OrderlistReview.splice(index, 1);
          });
        })
        .catch(() => {});
    },
